...
if ($devbox_bin | path exists) {
    try {
        let raw_env = (^$devbox_bin global shellenv -r | lines)
        
        # 'filter' 대신 'where'를 사용합니다.
        let env_record = ($raw_env 
        ...
        print "Devbox environment could not be loaded via custom parser."
    }
}
